...
/**
 * DI-friendly fact-source port. `LedgerFactSource` satisfies this structurally
 * (its `AuthoritativeContext` is assignable to `AuthoritativeFacts`).
 */
export interface FactSourcePort {
  contextFor(req: SpendRequest): AuthoritativeFacts | Promise<AuthoritativeFacts>;
}
...
export async function requestPurchase(
  ...
  const { request, kernel, kernelId, factSource, db, executor, idempotencyKey } =
    input;
  ...
  let facts: Facts;
  try {
    const authoritative = await factSource.contextFor(request);
