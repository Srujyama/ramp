...
function heroProvenance(facts: Facts = HERO_FACTS): FactProvenance[] {
  return [
    ...
    { fact: "agent_identity_verified", value: facts.agent_identity_verified, source: "identity", derivation: { kind: "agent_identity", agentId: "agent_47", scheme: "ed25519", keyRegistered: true, verified: true } },
...
test("factsDigest is key-order independent (canonical encoding)", () => {
  ...
  const reordered = {
    ...
    agent_identity_verified: HERO_FACTS.agent_identity_verified,
  budgets: [],
  recent_txn_count: 0,
  velocity_limit: 6,
  duplicate_recent_count: 0,
