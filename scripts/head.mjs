 ...
 *
 *   pnpm head                      # print a signed receipt (publish this)
 *   pnpm head --out receipt.json   # write it somewhere
 *
 * Then, later:
 *   pnpm proof --receipt receipt.json
 *
 * ============================================================================
 * WHERE YOU PUT THE RECEIPT IS THE WHOLE FEATURE
 * ============================================================================
 * This command is trivial. The hard part is not code, and no amount of code can
 * do it for you: THE RECEIPT MUST LIVE SOMEWHERE THE LEDGER'S OPERATOR CANNOT
 * REWRITE.
 *
 * A receipt saved next to the database is worthless — whoever rewrites the chain
 * rewrites the receipt in the same breath. `--out receipt.json` is for DEMOS and
 ...
 *
 * WHAT THE SIGNATURE IS FOR: it stops a third party fabricating a receipt to
...
try {
  ...
  const receipt = publishHead(db, demoGatePrivateKey(), DEMO_GATE_KEY_ID);
  ...
  if (out) {
    ...
    if (!quiet) {
      process.stderr.write(
        `\nwrote a receipt for ${length} decision(s) to ${out}\n` +
          `head ${receipt.statement.head.slice(0, 24)}…\n\n` +
          ...
          `Check it later:  pnpm proof --receipt ${out}\n\n`,
      );
    }
  } else {
